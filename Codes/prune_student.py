...
LR = 1e-5
NUM_LABELS = 20
OUTPUT_DIR = "pruned_student_model"
os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs('student_metrics', exist_ok=True)
...
original_model_teacher = AutoModelForSequenceClassification.from_pretrained(TEACHER_PATH).to(device)

print("Evaluating original teacher model...")
original_metrics_teacher = evaluate(original_model_teacher, test_dl, device)
original_size_teacher = count_parameters(original_model_teacher)
latency_orig_teacher = measure_latency(original_model_teacher, test_dl, device)
print("Teacher metrics: (Accuracy, Micro F1, Macro F1)", original_metrics_teacher)

print("Evaluating original student model...")
...
print("Fine-tuning (one epoch): Shallow re-training pruned student model...")
...
size_mb_orig = get_model_size_mb(MODEL_PATH)
size_mb_pruned = get_model_size_mb(OUTPUT_DIR)

size_mb_orig_teacher = get_model_size_mb(TEACHER_PATH)


df = pd.DataFrame([{
    "Model": "Teacher model",
    "Params": original_size_teacher,
    "Size_MB": size_mb_orig_teacher,
    "Latency_ms": latency_orig_teacher,
    "Acc": original_metrics_teacher[0],
    "Micro_F1": original_metrics_teacher[1],
    "Macro_F1": original_metrics_teacher[2],
    "Compression_Ratio": 1.0,
    "Speedup": 1.0
}, {

    "Model": "Original student",
    "Params": original_size,
    "Size_MB": size_mb_orig,
    "Latency_ms": latency_orig,
    "Acc": original_metrics[0],
    "Micro_F1": original_metrics[1],
    "Macro_F1": original_metrics[2],
    "Compression_Ratio": original_size_teacher/original_size,
    "Speedup": latency_orig_teacher/latency_orig
}, {
    "Model": "Pruned student model",
    "Params": pruned_size,
    "Size_MB": size_mb_pruned,
...
df.to_csv(os.path.join('student_metrics',"griffin_comparison.csv"), index=False)
